...
3. Gọi step-skill (Skill tool, chạy inline). Truyền: `RUN_DIR`; đường dẫn artifact **bước ngay trước** (nếu có); input đặc thù (bước 01: mô tả tính năng do người dùng cung cấp). Skill làm việc và ghi artifact vào `RUN_DIR` với front-matter `status: draft` (tên file do skill quy định).
