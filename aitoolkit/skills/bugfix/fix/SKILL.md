...
Read `shared/change-hygiene.md` before editing. Trace every edit to this bugfix task, distinguish new from existing files, record formatter commands, and inspect the final diff. Remove unrelated formatting or whole-file churn; local checkpoints must consolidate into one final delivery commit.

Orchestrator gọi skill này, truyền: `run_dir` + đường dẫn `<run_dir>/02-root-cause.md`. Chạy inline.
...
5. Ghi `<run_dir>/03-fix.md` (fix-report): nhánh, worktree, bảng file đã đổi, test bổ sung (trước/sau fix), ghi chú build và structured `Change Hygiene` gồm task-base SHA, file kind, edited region/symbol, formatter command, unrelated-diff verdict, checkpoint history. `status: draft`.
