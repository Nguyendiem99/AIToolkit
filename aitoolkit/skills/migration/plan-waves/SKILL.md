...
## Responsibility owner binding

Read `aitoolkit/contracts/file-responsibility-conformance.md` as the sole authority for `RESP-*`, verification-owner, co-location, and approval semantics; do not reproduce either responsibility matrix schema in this skill or in the plan.

For every selected work item, resolve the cited approved design revision from the bounded `revision: DESIGN-*@<positive integer>` field of the approved technical-design artifact and emit exactly one `Responsibility Owner References` row. The same revision must match the `Work Item Adapter Trace`; a plan-local revision is never authority. Preserve the exact ordered owner set from that design: concrete feature responsibilities in `Responsibility IDs`, `shared-foundation` owners only in `Shared Foundation IDs`, and integration/composition owners only in `Integration Responsibility IDs`.

Before approval, require a bidirectional one-to-one `Work Item ID` set between `Work Item Adapter Trace` and `Responsibility Owner References`, then resolve every `RESP-*` and its referenced `VERIFY-OWNER-*` from the approved design. Reject a missing, foreign, duplicate, stale, or cross-work-item-reused owner; never present a shared foundation as a concrete owner. Require immutable `Independent Boundary Evidence` beginning with a canonical rule, decision, or approval reference and demonstrating the selected owner set remains independently implementable, reviewable, verifiable, and revertible. Missing or non-exact binding yields `status: draft` and `result: blocked`.

When the orchestrator materializes or revises the executable master plan, it must emit exactly one bounded `responsibility_contract:` block with exact `version: 1` and `applicability: required`, then copy these approved `Responsibility Owner References` rows together with the corresponding `Delivery Adapter Selection` rows into that master-plan revision without mutation. A missing, pre-v1, unsupported, duplicate, or mixed plan discriminator keeps the revision non-executable. Initial queue authority is then derived from those two master-plan rows and the exact approved technical-design revision; this planning output must never create a synthetic pre-review handoff or queue-authority artifact.
...
5. For every unit specify dependencies, target area, acceptance, evidence, mode constraint, `Bootstrap Scope`, and exact responsibility owner references from the approved design revision. Allowed values are exactly `required | not-required`.
...
- Với generic adapter, không sinh ordered migration unit; emit exactly one `Work Item Adapter Trace` row with `Migration Unit ID = not-applicable`, then preserve canonical Work Item, master revision, dependencies via master-plan authority, acceptance, trace, and decomposition identity. The owner-reference row remains required and binds the generic work item to the same approved design revision.
...
- Preserve exactly one `Responsibility Owner References` row for every selected work item with its approved design revision, exact ordered concrete/shared/integration `RESP-*` sets, and independent-boundary evidence. Do not redefine the responsibility matrices.
