...
> AIToolKit là **Claude Code plugin thuần-prompt** (skill + slash command + manifest, KHÔNG script/runtime). Nó điều phối pipeline SDLC theo phong cách superpowers: mọi thứ là prompt + file `.md`.
...
```
/aitoolkit:migrate <workflow>          ┌─ đọc manifest (thứ tự bước)
        │  = CONDUCTOR (nhạc trưởng) ──┤   gọi từng step-skill
        │    chạy INLINE, giữ gate     └─ quản lý state.json (resume)
        ▼
...
2. **Pipeline** khai báo trong **manifest YAML**. Conductor chỉ biết manifest; step-skill chỉ biết artifact vào/ra của nó.

Hệ quả: **thêm workflow mới = thêm manifest + vài skill "nửa đầu", KHÔNG đụng conductor.**

**Skill vs Subagent:** *skill* = playbook nạp vào context; *subagent* = hộp chạy cách ly (context sạch, tốn token). Một bước là một **skill**; cờ `isolate: true` trong manifest bảo conductor bọc nó vào subagent.
...
├── commands/
│   ├── migrate.md                 # ★ CONDUCTOR (engine thật). Toàn bộ logic điều phối ở đây
│   ├── bugfix.md  feature.md       # wrapper mỏng: delegate sang conductor, ép sẵn workflow
├── workflows/
│   ├── migration.manifest.yaml    # 10 bước   ┐ khai báo: id, skill, isolate, optional, gate
│   ├── bugfix.manifest.yaml       #  9 bước   │ (nửa đầu đặc thù + nửa sau shared/*)
│   ├── feature.manifest.yaml      #  9 bước   │
│   └── _dryrun.manifest.yaml      #  5 stub   ┘ để test engine
├── skills/
│   ├── aitoolkit-schemas/         # ★ HỢP ĐỒNG DỮ LIỆU (artifact/manifest/state + project-profile). Đọc TRƯỚC
...
1. Conductor sinh `run_id`, tạo `<project>/.aitoolkit/run-<id>/`, ghi `state.json` (mọi bước `pending`).
2. Với mỗi bước theo thứ tự trong **manifest**: gọi skill (inline hoặc subagent theo `isolate`) → nhận đường dẫn artifact → cập nhật `state.json`.
3. Bước có `gate`: conductor **DỪNG**, trình artifact, hỏi qua `AskUserQuestion`. Duyệt → đi tiếp; Từ chối+feedback → chạy lại đúng bước đó.
4. `--resume run-<id>` bỏ qua bước đã `approved/skipped`, chạy tiếp từ bước dở. HARD gate không bao giờ tự vượt.
...
| # | Quy ước | Vì sao |
|---|---|---|
| I1 | **Conductor không nhúng logic của bước.** Chỉ đọc manifest + artifact + state. | Giữ seam 2; thêm workflow khỏi sửa engine |
| I2 | **Bước lấy input bước trước qua state.json, KHÔNG hardcode tên file.** Thứ tự nằm ở `manifest.steps` (mảng); id liền trước `current_step` → `state.json.steps[<prev_id>].artifact_path`. | `state.json.steps` là object không thứ tự; shared skill phải chạy được với mọi workflow |
...
| I7 | **Mỗi thay đổi trạng thái ghi ngay vào `state.json`.** | Để resume được |
...
2. **Viết manifest** `workflows/hotfix.manifest.yaml`: liệt kê bước nửa đầu rồi **tái dùng `shared/*`** cho nửa sau. Mỗi bước khai `id, skill, isolate, optional, gate`. (Tùy chọn thêm `change_type: bugfix|feature|migration` để verification chọn đúng chiến lược test.)
3. **Thêm template** cho artifact nửa đầu vào `templates/`.
4. **Wrapper command** (tùy chọn) `commands/hotfix.md`: delegate sang conductor, ép `workflow = hotfix` — copy `bugfix.md` làm mẫu. Nhớ **quote** frontmatter (I6).
5. `claude plugin validate ./aitoolkit` → **PASS mới merge**.

Xem manifest có sẵn (`bugfix.manifest.yaml`) làm mẫu chuẩn.

---

## 6. Language-agnostic: project-profile (cơ chế lai)

Shared skill lấy lệnh test/lint/build và mốc diff của repo theo thứ tự ưu tiên (định nghĩa ở `aitoolkit-schemas` §4):

1. **Khai báo** — file tùy chọn `<project>/.aitoolkit/project.yaml` (`test_cmd`, `lint_cmd`, `build_cmd`, `base_branch`, `review_focus`, `change_type`…). Team điền 1 lần.
...
|---|---|
| Spawn step-subagent (`isolate`) | `dispatching-parallel-agents`, `subagent-driven-development` |
...
- Kit thuần-prompt: **không** có Python/pytest — "test" nghĩa là review checklist + dry-run (`docs/DRY-RUN.md`), không phải chạy unit test.
...
**Đã xong (v0.5.0):** engine (conductor + gate + resume) chắc; 3 workflow (migration/bugfix/feature) chạy thật; `shared/ai-review` + `shared/verification-testing` đã sâu tới chuẩn superpowers + language-agnostic qua project-profile.
...
4. **Dry-run thật** cả 3 workflow trên một repo mẫu nhỏ + viết lại kết quả vào `docs/DRY-RUN.md`.
