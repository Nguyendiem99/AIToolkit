...
HEAD: f4c1a38ad9458f3a832cae99a77690c5d1c7bd72

## Commits
f4c1a38 feat: block terminal scope on responsibility defects

## Stat
 .../aitoolkit/skills/aitoolkit/migrate/SKILL.md    |  30 +++-
 .../templates/migration/scope-terminal-report.md   |  14 +-
 .../tests/scenarios/flexible-scope-e2e.Tests.ps1   |  93 ++++++++++--
 .../scenarios/responsibility-handoff.Tests.ps1     |  61 ++++++++
 .../tests/scenarios/scope-engine.Tests.ps1         | 167 ++++++++++++++++++++-
 .../tests/validate-migration-framework.ps1         | 137 ++++++++++++++---
 .../validation/architecture-review.validation.ps1  |  79 ++++++++++
 .../tests/validation/scope-engine.validation.ps1   | 164 ++++++++++++++++++--
 8 files changed, 694 insertions(+), 51 deletions(-)
...
@@ -70,45 +70,67 @@ Ngay trước execution, atomically đổi selected item `ready -> in-progress`,
...
@@ -20,29 +20,35 @@ produced_at: <YYYY-MM-DD>
...
+| EVIDENCE-ARCH-WORK-<PHẠM-VI>-<TÊN> | <same immutable terminal evidence reference used by the work-item row and responsibility handoff> | WORK-<PHẠM-VI>-<TÊN> | architecture-responsibility-sub-verdicts |
...
@@ -92,22 +111,38 @@ function Assert-PredecessorReferenceRejected([object]$Result,[string]$ScenarioId
...
+  $terminalA=Add-ResponsibilityHandoff $run 'terminal-a.md' $reviewA
+  $terminalB=Add-ResponsibilityHandoff $run 'terminal-b.md' $reviewB
...
@@ -260,72 +295,79 @@ produced_at: 2026-08-20
...
+  [pscustomobject]@{ Run=$run; Manifest=$manifestPath; ReviewA=$reviewA; ReviewB=$reviewB; TerminalA=$terminalA; TerminalB=$terminalB; HistoricalA=$historicalA; HistoricalB=$historicalB; ResponsibilityChains=$responsibilityChains }
 }
...
@@ -438,37 +480,65 @@ foreach($scenario in $scenarios){
...
@@ -586,22 +656,25 @@ $priorActivationTable
...
@@ -86,11 +92,66 @@ Assert-HandoffRejected 'rejects a downstream artifact with no responsibility han
...
+    foreach ($artifactListField in @('terminal_artifacts', 'responsibility_evidence_artifacts')) {
...
@@ -109,28 +114,70 @@ function New-WorkItem(
     work_item_id = $Id
     required = $Required
...
@@ -291,36 +338,78 @@ Assert-Equal $ordered.result 'selected' 'Valid graph must select one item'
...
@@ -513,39 +602,115 @@ $partialCompletion = Invoke-ScopeScenario @{
   scenario_type = 'scope-engine'
...
+$completeTerminal = New-TerminalResponsibilityArtifact $completeItem.terminal_evidence $completeItem.work_item_id $completeItem.status $completeReview.artifact_reference
+$cancelledTerminal = New-TerminalResponsibilityArtifact $cancelled.terminal_evidence $cancelled.work_item_id $cancelled.status $cancelledReview.artifact_reference
+$notApplicableTerminal = New-TerminalResponsibilityArtifact $notApplicable.terminal_evidence $notApplicable.work_item_id $notApplicable.status $notApplicableReview.artifact_reference
...
+  responsibility_evidence_artifacts = @($completeReview, $cancelledReview, $notApplicableReview)
...
+      verification_ownership = 'PASS'
+      architecture_state = 'PASS'
+      evidence_references = @('runs/complete.md', 'decisions/cancelled.md', 'decisions/not-applicable.md')
+    }
     items = @(
...
+$missingV1Terminal = New-TerminalResponsibilityArtifact $completeItem.terminal_evidence $completeItem.work_item_id $completeItem.status $completeReview.artifact_reference
...
+
+$mixedVersionTerminal = New-TerminalResponsibilityArtifact $completeItem.terminal_evidence $completeItem.work_item_id $completeItem.status $completeReview.artifact_reference
...
@@ -1535,32 +1535,98 @@ function Test-FlexibleScopeFixtureEnvelope([string]$FixturePath) {
     }
...
+    if($declaredChains.Count-eq0){$responsibilityChainValid=$false}
...
+      if(($chainFields-join'|')-cne'artifact_refs|work_item_id'-or[string]::IsNullOrWhiteSpace($workItemId)-or$responsibilityChainByWorkItem.ContainsKey($workItemId)-or$artifactReferences.Count-ne5){$responsibilityChainValid=$false;continue}
...
+      }
+      if($chainTexts.Count-ne5){$responsibilityChainValid=$false;continue}
...
+      if($initialHandoff.Count-ne1-or$initialHandoff[0].'Evidence References'-cnotmatch('^source-diff:[0-9a-f]{40}\.\.[0-9a-f]{40}#'+[regex]::Escape($workItemId)+'$')){$responsibilityChainValid=$false}
+      for($chainIndex=0;$chainIndex-lt4;$chainIndex++){
...
+      }
+      $finalHandoff=@(& $table $chainTexts[4] 'Architecture Responsibility Handoff' @('Responsibility Contract Version','Tree Conformance','Responsibility Conformance','Verification Ownership','Architecture Conformance State','Evidence References') "Responsibility terminal chain $workItemId")
...
+      $responsibilityChainByWorkItem[$workItemId]=[pscustomobject]@{FinalReference=$chainReferences[4];Handoff=$finalHandoff[0]}
+    }
+  }
...
+    $fm=&$frontMatter $text "$Context responsibility evidence"
...
+    if((($keys-join'|')-cne'approval_source|produced_at|result|status|step_id')-or$fm.step_id-cne'11-ai-review'-or$fm.status-cne'approved'-or$fm.result-cne'complete'-or$fm.approval_source-cne'human'-or$fm.produced_at-cnotmatch'^20[0-9]{2}-[0-9]{2}-[0-9]{2}$'-or$provenance.Count-ne1-or$handoff.Count-ne1){return $false}
...
@@ -1679,20 +1745,22 @@ function Test-FlexibleScopeFixtureEnvelope([string]$FixturePath) {
...
+    $responsibilityEvidenceValid=$true
     $valid=(
...
@@ -1700,21 +1768,21 @@ function Test-FlexibleScopeFixtureEnvelope([string]$FixturePath) {
...
@@ -1853,45 +1921,58 @@ function Test-FlexibleScopeFixtureEnvelope([string]$FixturePath) {
...
+      if($terminalSuccess){$terminalBinding=& $resolveImmutableEvidence $r[0].'Terminal Evidence' $p.'Work Item ID' $planFm.revision $p.'Trace IDs' $p.Acceptance $p.'Delivery Adapter' $p.'Approval Reference' ([string]$fixture.master_spec_ref) ([string]$fixture.master_plan_ref) 'Terminal scope report';if(-not$terminalBinding.Valid-or$r[0].'Runtime Evidence State'-cne$terminalBinding.Runtime-or$r[0].'Architecture Conformance State'-cne$terminalBinding.Architecture-or$r[0].'Selector Schema State'-cne$terminalBinding.Selector-or$null-eq$chainEntry){$valid=$false}else{$resolvedResponsibilityTerminals.Add([string]$chainEntry.FinalReference)}}elseif($r[0].'Terminal Evidence'-cne'none'){$valid=$false}
...
@@ -1923,30 +2004,39 @@ function Test-FlexibleScopeIntegration([bool]$InvokeModules) {
...
+      'same immutable terminal evidence reference used by the work-item row and responsibility handoff',
...
@@ -6367,20 +6457,31 @@ function Test-Skills {
     'Test-StructuralGate' `
...
@@ -7378,26 +7479,28 @@ function Test-Orchestrators {
...
@@ -331,40 +331,76 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
...
@@ -522,20 +553,123 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
         ) {
...
+        }
+
+        $handoff = $terminalArtifacts[0].responsibility_handoff
...
+        ) { 'PASS' } else { 'BLOCKED' }
+        $responsibilityEvidenceReference = [string]$handoff.evidence_reference
+        if (
+          $null -eq $handoff -or
...
+          @('', 'pending', 'none', 'not-applicable') -ccontains $responsibilityEvidenceReference -or
...
+        }
+
+        $responsibilityEvidence = @($scenario.orchestration_context.resolved_artifacts | Where-Object {
+          [string]$_.artifact_reference -ceq $responsibilityEvidenceReference
+        })
+        if (
+          $responsibilityEvidence.Count -ne 1 -or
+          -not [bool]$responsibilityEvidence[0].immutable -or
+          [string]$responsibilityEvidence[0].artifact_type -cne 'architecture-responsibility-review' -or
+          [string]$responsibilityEvidence[0].work_item_id -cne [string]$item.work_item_id -or
+          [int]$responsibilityEvidence[0].responsibility_contract_version -ne 1 -or
+          [string]$responsibilityEvidence[0].tree_conformance -cne [string]$handoff.tree_conformance -or
+          [string]$responsibilityEvidence[0].responsibility_conformance -cne [string]$handoff.responsibility_conformance -or
+          [string]$responsibilityEvidence[0].verification_ownership -cne [string]$handoff.verification_ownership -or
+          [string]$responsibilityEvidence[0].architecture_state -cne [string]$handoff.architecture_state -or
+          [string]::IsNullOrWhiteSpace([string]$responsibilityEvidence[0].evidence_reference) -or
+          [string]$responsibilityEvidence[0].evidence_reference -ceq 'none'
+        ) {
+          return [pscustomobject]@{ result = 'scope-not-complete'; reason = 'structural-assurance-blocked'; scope_status = 'scope-blocked' }
+        }
+        $resolvedTerminalReferences.Add([string]$item.terminal_evidence)
+      }
