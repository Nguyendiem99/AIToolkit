# Task 8 final round-4 review package

BASE: 8c0e78b51e79006867f8f08a4796c44a3242eccf
HEAD: 548756e2ab8089c391c87471eaab138a50532f7d

## Commits
548756e feat: block terminal scope on responsibility defects

## Stat
 .../aitoolkit/skills/aitoolkit/migrate/SKILL.md    |  36 +-
 .../templates/migration/scope-terminal-report.md   |  16 +-
 .../tests/scenarios/flexible-scope-e2e.Tests.ps1   | 230 ++++++-
 .../scenarios/responsibility-handoff.Tests.ps1     |  70 ++
 .../tests/scenarios/scope-engine.Tests.ps1         | 718 ++++++++++++++++++++-
 .../tests/validate-migration-framework.ps1         | 369 ++++++++++-
 .../validation/architecture-review.validation.ps1  |  82 +++
 .../tests/validation/scope-engine.validation.ps1   | 436 ++++++++++++-
 8 files changed, 1869 insertions(+), 88 deletions(-)
...
+  $path=Join-Path $Run $Name
+  Write-Utf8 $path "---`nstep_id: $StepId`nstatus: approved`nresult: complete`napproval_source: human`nproduced_at: 2026-08-20`nresponsibility_contract:`n  version: 1`n  applicability: required`n---`n# Responsibility Handoff Stage`n`n## Master Scope Context`n`n| Run ID | Master Spec Reference | Master Spec ID | Master Spec Revision | Master Plan Reference | Master Plan ID | Master Plan Revision | Work Item ID |`n|---|---|---|---|---|---|---|---|`n| RUN-E2E-001 | rendered-scope-run/master-spec.md | SPEC-E2E-001 | 1 | rendered-scope-run/master-plan.md | PLAN-E2E-001 | 1 | $WorkItem |`n`n## Task Provenance`n`n| Task / Unit | Task-base SHA | Final-tree SHA | Source Artifact |`n|---|---|---|---|`n| $WorkItem | 1111111111111111111111111111111111111111 | 2222222222222222222222222222222222222222 | $SourceArtifact |`n`n## Architecture Responsibility Handoff`n`n| Responsibility Contract Version | Tree Conformance | Responsibility Conformance | Verification Ownership | Architecture Conformance State | Evidence References |`n|---|---|---|---|---|---|`n| 1 | PASS | PASS | PASS | PASS | source-diff:1111111111111111111111111111111111111111..2222222222222222222222222222222222222222#$WorkItem |`n"
+  Get-ImmutableReference $Run $Name
+}
...
@@ -438,57 +520,174 @@ foreach($scenario in $scenarios){
...
+
 ...
 |---|---|---|---|---|---|---|
@@ -528,21 +727,21 @@ $priorActivationTable
 ## $UnknownHeading
...
@@ -586,22 +785,25 @@ $priorActivationTable
...
@@ -753,62 +1270,219 @@ $sameItemSecondStart = Invoke-ScopeScenario @{
 }
...
+$nativeBlockerSibling.responsibility_evidence = 'none'
...
+  @{ Name='missing'; Mutate={ param($item, $authority) $item.responsibility_evidence = 'none'; return $null } },
+  @{ Name='pre-v1'; Mutate={ param($item, $authority) [void]$authority.Remove('responsibility_handoff'); return $authority } },
...
+    }
+    $startBlockerAuthority = New-QueueResponsibilityArtifact $startBlocker.responsibility_evidence $startBlocker
...
+    }
+    if ($null -ne $startBlockerAuthority) { $startFixture.responsibility_authority_artifacts = @($startBlockerAuthority) }
...
+    Assert-Equal $startResult.reason 'structural-assurance-blocked' "Attempt start must derive a structural blocker from $executableClass B's $($authorityCase.Name) authority"
...
+    }
+    $successBlockerAuthority = New-QueueResponsibilityArtifact $successBlocker.responsibility_evidence $successBlocker
...
+    }
+    if ($null -ne $successBlockerAuthority) { $successFixture.responsibility_authority_artifacts = @($successBlockerAuthority) }
...
+    Assert-Equal $successResult.reason 'structural-assurance-blocked' "Successful transition must derive a structural blocker from $executableClass B's $($authorityCase.Name) authority"
...
+$missingTransitionStartAuthority.responsibility_evidence = 'none'
...
+Assert-Equal $blockedTransitionStart.reason 'structural-assurance-blocked' 'Missing transition-start authority must derive the structural blocker'
...
+  $caseAuthority = New-QueueResponsibilityArtifact $caseItem.responsibility_evidence $caseItem
...
+  }
+  if ($null -ne $caseAuthority) { $caseFixture.responsibility_authority_artifacts = @($caseAuthority) }
...
+  Assert-Equal $caseResult.reason 'structural-assurance-blocked' "Transition $($authorityCase.Name) authority must derive BLOCKED"
...
@@ -1535,41 +1535,303 @@ function Test-FlexibleScopeFixtureEnvelope([string]$FixturePath) {
     }
...
@@ -1679,42 +1941,52 @@ function Test-FlexibleScopeFixtureEnvelope([string]$FixturePath) {
...
@@ -1769,46 +2041,39 @@ function Test-FlexibleScopeFixtureEnvelope([string]$FixturePath) {
     ){return $false}
...
@@ -1853,45 +2118,68 @@ function Test-FlexibleScopeFixtureEnvelope([string]$FixturePath) {
...
@@ -1923,30 +2211,40 @@ function Test-FlexibleScopeIntegration([bool]$InvokeModules) {
...
@@ -6367,20 +6665,31 @@ function Test-Skills {
     'Test-StructuralGate' `
...
@@ -7378,26 +7687,28 @@ function Test-Orchestrators {
...
@@ -171,20 +171,215 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
...
+    $testQueueResponsibilityAuthority = {
+      param([object]$Item)
...
+        [string]$authorityHandoff.architecture_state -ceq $derivedArchitecture -and
+        $derivedArchitecture -ceq 'PASS' -and
...
+        if (-not ([bool]$candidateItem.required -or [bool]$candidateItem.optional_execution_approved)) { continue }
+        if (-not (& $testQueueResponsibilityAuthority $candidateItem)) {
...
+            $candidateItem | Add-Member -NotePropertyName responsibility_diagnostic -NotePropertyValue 'structural-assurance-blocked' -Force
+          }
+        }
+      }
+
...
@@ -300,20 +495,23 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
       }
...
@@ -348,23 +546,30 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
...
+        $assurancePass = $architecturePass -and $item.selector_schema_state -ceq 'PASS'
         if (
           ...
           continue
@@ -400,23 +605,29 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
...
+          } else { 'required-work-remains-without-eligible-item' }
       ...
       return [pscustomobject]@{
         ...
         work_item_id = ''
@@ -475,31 +686,20 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
...
@@ -522,20 +722,215 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
         ) {
...
@@ -556,20 +951,23 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
     ...
     if ($scenario.operation -ceq 'transition') {
       ...
       $item = $selected[0]
+      if (-not (& $testQueueResponsibilityAuthority $item)) {
+        return [pscustomobject]@{ result = 'transition-invalid'; reason = 'structural-assurance-blocked'; scope_status = 'scope-blocked' }
+      }
...
@@ -592,20 +990,24 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
           ) {
...
@@ -648,25 +1050,29 @@ function Test-ScopeEngine([string]$Root, [string]$ContractText) {
         $terminalArtifacts = @($scenario.orchestration_context.resolved_artifacts | Where-Object {
